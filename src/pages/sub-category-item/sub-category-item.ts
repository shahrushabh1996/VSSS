...
/**
 * Generated class for the SubCategoryItemPage page.
 *
...
@IonicPage()
@Component({
  selector: 'page-sub-category-item',
  templateUrl: 'sub-category-item.html',
})
export class SubCategoryItemPage {
  ...
  SubCategoryNameGujarati: string;
  ...
  constructor(public navCtrl: NavController, public navParams: NavParams, public device: Device, public http: Http, public platform: Platform, public loadingCtrl: LoadingController, public socialSharing: SocialSharing, public headerColor: HeaderColor, public translate: TranslateService, public barcodeScanner: BarcodeScanner) {
    ...
    this.sub_category = navParams.get('id');
  }

  ionViewDidLoad() {
    console.log('ionViewDidLoad SubCategoryItemPage');
  }
  ...
  get_sub_category_item(){
  	var link = 'https://www.vsss.co.in/Android/get_sub_category_item';
    var post_data = JSON.stringify(
        {
    ...
    this.http.post(link, post_data).map(res => res.json()).subscribe(data => {
        this.SubCategoryName = data['Name'].toLowerCase();
        this.SubCategoryNameGujarati = data['Name_gujarati'];
        this.Loader = 0;
        this.ion_content = '';
    });
  }
